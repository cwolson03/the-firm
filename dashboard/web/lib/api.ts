...
export const fetchStatus = () => get<StatusResponse>("/api/status", "/data/status.json");
export const fetchActivity = (n = 100, agent = "") =>
  get<ActivityEntry[]>(`/api/activity?n=${n}${agent ? `&agent=${agent}` : ""}`);
export const fetchPositions = () => get<PositionItem[]>("/api/positions", "/data/positions.json");
export const fetchWeather = () => get<WeatherResponse>("/api/weather", "/data/weather.json");
export const fetchBrad = () => get<BradResponse>("/api/brad", "/data/brad.json");
export const fetchEval = () => get<EvalRecord[]>("/api/eval", "/data/eval.json");
...
export const fetchPortfolio = () => get<PortfolioItem[]>("/api/portfolio", "/data/portfolio.json");
