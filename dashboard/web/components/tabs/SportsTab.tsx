...
const STRATEGY_META: Record<string, { full: string; desc: string }> = {
  S1: { full: 'S1 — Live Game Winners', desc: 'Real-time game state analysis' },
  S2: { full: 'S2 — Spread & Prop Markets', desc: 'Line value + prop bets' },
  S3: { full: 'S3 — Tournament Outrights', desc: 'Futures and tournament picks' },
}
...
export default function SportsTab() {
  ...
  const recentResolved = (data.recent || []).filter((t: Record<string, unknown>) => {
    const status = String(t.status || '')
    return status.includes('win') || status.includes('loss') || status === 'won' || status === 'lost' ||
           status === 'expired_win' || status === 'expired_loss'
  }).slice(0, 20)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <StatCard label="Best Strategy" value={bestStrat ? (STRATEGY_META[bestStrat[0]]?.full || bestStrat[0]) : '—'} sub={bestStrat ? `${bestStrat[1].win_rate.toFixed(1)}% win rate` : ''} color="#00ff88" />
      </div>

      {/* Strategy breakdown */}
      <div className="grid grid-cols-3 gap-3">
        {Object.entries(data.by_strategy || {}).map(([key, s]) => {
          const meta = STRATEGY_META[key] || { full: key, desc: '' }
          return (
            <div key={key} className="border border-[#222] rounded-lg bg-[#111] p-3">
              <div className="flex items-center gap-2 mb-1">
                <span className="text-xs font-medium text-[#e5e5e5]">{meta.full}</span>
              </div>
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-[#222]">
                  <th className="text-left py-2 px-2 text-[#888]">Placed</th>
                  <th className="text-left py-2 px-2 text-[#888]">Title</th>
                  <th className="text-center py-2 px-2 text-[#888]">Strategy</th>
                  <th className="text-right py-2 px-2 text-[#888]">Bid</th>
                  <th className="text-center py-2 px-2 text-[#888]">Status</th>
                </tr>
              </thead>
              <tbody>
                {openTrades.map((t: Record<string, unknown>, i: number) => {
                  const title = String(t.title || t.market || t.ticker || '—').slice(0, 50)
                  const status = String(t.status || 'open').toLowerCase()
                  const strategy = String(t.strategy || '—')
                  const stinkPrice = t.stink_price_c ?? t.bid_price ?? t.price
                  return (
                    <tr key={i} className="border-b border-[#1a1a1a]">
                      <td className="py-1.5 px-2 text-[#666] whitespace-nowrap">{formatTimestamp(String(t.timestamp || ''))}</td>
                      <td className="py-1.5 px-2 text-[#e5e5e5]">{title}</td>
                      <td className="py-1.5 px-2 text-center">
                        <span className="text-[10px] text-[#888]">{STRATEGY_META[strategy]?.full || strategy}</span>
                      </td>
                      <td className="py-1.5 px-2 text-right text-[#e5e5e5] font-mono">
                        {stinkPrice != null ? `${stinkPrice}¢` : '—'}
                      </td>
                      <td className="py-1.5 px-2 text-center">
      ...
      <div className="border border-[#222] rounded-lg bg-[#111] p-4">
        ...
        <DataTable
          columns={[
            { key: 'date', label: 'Date', render: (_: unknown, row: Record<string, unknown>) =>
              (String(row.date || row.timestamp || '—')).slice(0, 10)
            },
            { key: 'market', label: 'Market', render: (_: unknown, row: Record<string, unknown>) => (
              <span className="truncate block max-w-[200px]">{String(row.title || row.market || '—').slice(0, 45)}</span>
            )},
            { key: 'strategy', label: 'Strategy', render: (_: unknown, row: Record<string, unknown>) => {
              const s = String(row.strategy || '—')
              return <span className="text-[10px]">{STRATEGY_META[s]?.full || s}</span>
            }},
            { key: 'bid', label: 'Bid', render: (_: unknown, row: Record<string, unknown>) => {
              const bp = row.stink_price_c ?? row.bid_price
              return bp != null ? `${bp}¢` : '—'
            }},
            { key: 'result', label: 'Result', render: (_: unknown, row: Record<string, unknown>) => {
              const status = String(row.status || '')
              const isWin = status.includes('win') || status === 'won'
              return (
                <span className={`px-2 py-0.5 rounded text-[10px] font-medium ${
