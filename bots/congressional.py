...
HEADERS = {"User-Agent": "CongressionalBot/2.0 (congressional-trade-tracker; contact via Discord)"}
...
def format_high_conviction_alert(trade: dict, scored: dict, stock_data: dict, news: str) -> str:
    """Build the full CONGRESSIONAL ALERT post for HIGH CONVICTION trades."""
    ...
    lines = [
        f"🏛️ **CONGRESSIONAL ALERT** — Congressional Trade Detected",
        f"",
...
def format_watch_alert(trade: dict, scored: dict) -> str:
    ...
    return (
        f"🟡 **CONGRESSIONAL MONITOR** — {member_name} ({party}) | "
...
def process_trades(trades: list, days: int = 7, demo: bool = False, force_new: bool = False) -> list:
    ...
    if note_trades:
        lines = [f"📋 **CONGRESSIONAL — Notable Trades (NOTE tier, {len(note_trades)} trades):**"]
...
def run_scan(demo: bool = False):
    ...
    if not processed:
        msg = "🏛️ **CONGRESSIONAL** — Scan complete. No new trades from watched members in last 7 days."
        post_discord(CHANNEL_SENATOR_TRACKER, msg, demo=demo)
...
def run_recent(demo: bool = False, post: bool = True):
    ...
    try:
        ...
        _cs.path.insert(0, _co.path.dirname(_co.path.abspath(__file__)))
        from shared_context import write_agent_status
        write_agent_status('congressional', {'status': 'ran', 'trades_processed': len(processed)})
    except Exception:
        pass
    if not processed:
...
def run_member(member_query: str, demo: bool = False):
    ...
    lines = [
        f"🏛️ **CONGRESSIONAL — {matched_key} Trade History**",
...
def run_summary(demo: bool = False):
    ...
    lines = [
        f"📊 **CONGRESSIONAL WEEKLY INTELLIGENCE REPORT — Week of {now}**",
