...
============================================
Stratton Oakmont prediction market intelligence — operational watchdog.
...
Checks:
  ...
  4. Unexpected live orders on Kalshi (brad-* should be paper)
  5. Paper mode integrity (brad.py, weather.py)
...
# ─────────────────────────────────────────────────────────────────────────────
# AUTH — RSA-PSS (copied exactly from donnie-v3)
# ─────────────────────────────────────────────────────────────────────────────
...
def check_unexpected_orders(state: dict) -> Optional[tuple]:
    ...
    - brad-* prefix orders (Brad should be paper)
    ...
    try:
        ...
        for o in orders:
            ...
            # Brad orders should be paper (brad should not place real Kalshi orders)
        ...
        if brad_live:
            ...
            if should_send_alert(state, key):
                order_list = "\n".join(brad_live)
                alerts.append((key, f"🚨 BRAD IS LIVE: {len(brad_live)} real resting order(s) found — "
                                    f"Brad should be in paper mode!\n```\n{order_list}\n```\n"
                                    f"Action: Cancel orders and check brad.py run_scan()"))
...
def check_paper_mode_integrity(state: dict) -> Optional[tuple]:
    """
    Read brad.py and weather.py source to verify paper mode is intact.
    ...
    # Brad check
    try:
        if os.path.exists(BRAD_PY_PATH):
            with open(BRAD_PY_PATH) as f:
                brad_content = f.read()

            if 'paper=False' in brad_content and 'paper=True' not in brad_content:
                issues.append("⚠️ PAPER MODE BROKEN: brad.py has paper=False but no paper=True anywhere")
            else:
                log.info("  brad.py paper mode: OK")
        else:
            log.info(f"  brad.py not found at {BRAD_PY_PATH}")
    except Exception as e:
        log.error(f"  Brad paper mode check error: {e}")
    ...
    try:
        if os.path.exists(WEATHER_PY_PATH):
            ...
            # Check for dry_run=True or paper=True in weather.py
            has_dry_run_true  = 'dry_run=True' in weather_content or 'paper=True' in weather_content
            has_dry_run_false = 'dry_run=False' in weather_content or 'paper=False' in weather_content

            if has_dry_run_false and not has_dry_run_true:
                issues.append("⚠️ PAPER MODE BROKEN: weather.py has dry_run=False/paper=False but no dry_run=True/paper=True")
            else:
                log.info("  weather.py paper mode: OK")
        else:
