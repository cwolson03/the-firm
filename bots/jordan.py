...
Discord group sends CALL/PUT + price target → Cody executes → Jordan monitors.
"""
...
def _format_open_positions(open_positions: list, spy_price: float) -> str:
    ...
    for p in open_positions:
        ...
        opt    = p.get('type', '?')
        strike = p.get('strike', '?')
        source_tag = f" [{p['source']}]" if p.get('source') != 'manual' else ""
...
def analyze_discord_alert(alert_type: str, price_target: float) -> str:
    ...
    if not spy_price:
        return "❌ Jordan: Can't pull SPY data right now. Try again."
    ...
    lines.extend([
        ...
        "",
        f"_Log it with: `python3 bots/jordan.py --add {alert_type} <STRIKE> <ENTRY> --target {price_target:.2f} --source discord_group`_",
        f"*Jordan | The Firm*",
    ])
...
# ── Entry point ─────────────────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description='Jordan — SPY 0DTE Options Desk')
