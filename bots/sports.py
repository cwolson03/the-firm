...
======================================================
The Firm — Kalshi sports prediction market scanner.
...
Usage:
    python3 sports.py                         # paper mode continuous (default — safe)
    python3 sports.py --live                  # LIVE mode — real orders
    python3 sports.py --scan-once             # paper single scan + exit
    python3 sports.py --live --scan-once      # live single scan + exit
    python3 sports.py --dry-run               # stdout only, no orders, no Discord
    python3 sports.py --paper                 # explicit paper mode (same as default)

firm.py calling run_scan() → always LIVE.

Auth: RSA-PSS (same as Economics engine). Paths auto-detect Atlas vs local.
"""
...
# run_scan() called by firm.py — LIVE mode enabled.
...
BRAD_MAX_BALANCE_PCT   = 0.25    # Sports hard cap: max 25% of account balance in stink bids
...
def _load_brad_token() -> str:
    # Use Sports own token; fall back to Economics if missing
...
def get_all_open_positions_exposure() -> float:
    ...
    in the shared Kalshi account (Sports + Economics combined).
    Sports must see the full picture before placing new bids.
...
def get_open_brad_orders() -> dict:
    """
    Fetch all resting orders placed by Sports (client_order_id starts with 'brad-').
...
def get_open_brad_orders_by_strategy(paper: bool = False) -> dict:
    """
    Fetch all resting Sports orders grouped by strategy prefix.
...
def get_filled_brad_orders() -> list:
    """
    Check for recently filled Sports orders (for fill notifications).
...
def get_total_brad_exposure(open_orders: dict) -> float:
    """Calculate total dollar exposure from open Sports stink bids."""
    total = 0.0
...
def refresh_stink_bids(open_orders: dict, markets: list, balance: float, dry_run: bool = False):
    """
    Cancel and re-place each open Sports stink bid at the current market price * discount.
    Keeps Sports competitive as prices shift during live games.
    ...
    for trade in trades:
        ...
        if expired and trade["status"] == "filled":
            ...
            if result_val:
                ...
                changed = True
                # log to eval framework for process scoring
                try:
                    import sys as _ss; import os as _so
                    _ss.path.insert(0, _so.path.dirname(_so.path.abspath(__file__)))
                    from eval_framework import log_trade_entry as _sef, resolve_trade as _srt
                    _srt(
                        trade_id=trade.get("id", ticker),
                        outcome="WIN" if won else "LOSS",
                        pnl_pct=round(pnl / max(cost, 0.01) * 100, 1)
                    )
                except Exception:
                    pass  # never blocks
...
def check_and_notify_fills(dry_run: bool = False):
    """Check for newly filled Sports orders and post Discord notifications."""
...
def run_brad_scan(dry_run: bool = False, paper: bool = True):
    ...
    if brad_total_exposure >= brad_hard_cap:
        log.info(
            f"[Sports] Sports hard cap reached: ${brad_total_exposure:.2f} >= ${brad_hard_cap:.2f} — "
...
def run_brad_refresh(dry_run: bool = False, paper: bool = True):
    """
    Cancel and re-place all open Sports stink bids at updated prices.
    ...
    if not open_orders:
        log.info("[Sports] No open Sports orders to refresh")
        return
    ...
    if not open_orders:
        log.info("[Sports] No remaining open Sports orders after cricket cancellation")
        return
...
def run_scan(post=None, **kwargs):
    """Entry point for firm.py orchestrator."""
    run_brad_scan(dry_run=False, paper=SPORTS_PAPER_MODE)
    try:
        import sys as _ss, os as _so
        _ss.path.insert(0, _so.path.dirname(_so.path.abspath(__file__)))
        from shared_context import write_agent_status
        write_agent_status('sports', {'status': 'ran', 'mode': 'paper' if SPORTS_PAPER_MODE else 'live'})
    except Exception:
        pass
